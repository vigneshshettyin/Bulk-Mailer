...
class User(db.Model, UserMixin):
    ...
    templates = db.relationship(
        'Template', cascade="all,delete", backref='templates')
        ...
        if(password != password2):
            ...
            if not response:
                ...
                entry = User(name=name, email=email, password=password,
                             date=time, profile_image=profile_image, status=1, is_staff=1)
                ...
                if isinstance(response, bool) and not response:
                    ...
                    flash(
                        "Now verify your email address for activating your account.", "success")
...
def subscribers_page(number):
    ...
    post = Subscriber.query.filter_by(
        group_id=number).filter_by(user_id=current_user.id).all()
...
def submit_new_subscribers():
    if(request.method == 'POST'):
        ...
        entry = Subscriber(email=email, date=time,
...
def add_template():
    ...
    if (request.method == 'POST'):
        ...
        entry = Template(name=name, date=time, content=editordata,
