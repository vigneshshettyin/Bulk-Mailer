...
def validate(vtype=None, text=None):
    ...
    if not isinstance(vtype, str):
        print(
            "Error: Invalid argument passed to 'vtype'. Required 'str', found {}".format(
                type(vtype)
            )
        )
        return None
    ...
    if vtype not in VALIDATION_TYPES:
        print(
            "Error: Invalid argument passed to 'vtype'. Can only be one of: {}".format(
    ...
    if not isinstance(text, str):
        print(
            "Error: Invalid argument passed to 'text'. Required 'str', found {}".format(
                type(text)
            )
        )
        return None

    # Commented this so that the if password is empty,
    # the function would not return here but it will return False in the Validation test
    #  Test that the vtype argument is among the available Types
    # if len(text) == 0:
